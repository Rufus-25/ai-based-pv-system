...
// ===== Base AI Function =====
int getPredictedAngle(int month, int hour, int minute) {
  if (hour < 6 || hour > 18) {
    return 90;
  }
  
  // If it's before the half hour (minute < 30), use current hour's value
  if (minute < 30) {
    return lookupTable[month - 1][hour];
  }
  // If it's after the half hour (minute >= 30), average current and next hour
  else {
    // Make sure we don't go beyond the array bounds (hour 18 is the last valid hour)
    if (hour < 18) {
      int currentHourAngle = lookupTable[month - 1][hour];
      int nextHourAngle = lookupTable[month - 1][hour + 1];
      return (currentHourAngle + nextHourAngle) / 2;
    } else {
      // If it's hour 18 and after 30 minutes, just use hour 18's value
      return lookupTable[month - 1][hour];
    }
  }
}
...
void loop() {
  ...
  int minute = p_tm->tm_min;  // Add this line
  ...
  int newAngle = getPredictedAngle(month, hour, minute);
